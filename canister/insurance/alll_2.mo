...
// Modifica della funzione createClaim per gestire i crediti
public shared({ caller }) func createClaim(
    ...
    switch(users.get(caller)) {
        case (null) #err("User must be registered before creating a claim");
        case (?user) {
                ...
                case (#ok()) {
                    ...
                    switch(claims.get(uniqueClaimKey)) {
                        ...
                        case (null) {
                            ...
                            users.put(caller, updatedUser);
                            #ok("Claim created successfully. Remaining claim credits: " # Nat.toText(updatedUser.credits.claimCredits))
                        };
                    }
                };
            }
